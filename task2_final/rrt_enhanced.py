"""
rrt_enhanced.py — Enhanced Hybrid APF-RRT with two Phase B improvements:

  1. **Adaptive Parameter Tuning** (heuristic, density-aware)
     Step size and APF weights (K_att, K_rep) are dynamically adjusted
     based on local obstacle density and stall detection to escape
     local minima.

  2. **Optimization-Based Path Smoothing** (gradient descent + stochastic shortcutting)
     After a raw path is found, a two-stage optimizer minimises total
     joint-space path length subject to collision-free constraints:

     Stage 1 — Stochastic shortcutting:
       Randomly sample pairs (i, j) of non-adjacent waypoints; if the
       straight-line segment q_i → q_j is collision-free, remove all
       intermediate waypoints.  This is a stochastic combinatorial
       optimiser over the set of waypoint subsets.

     Stage 2 — Gradient descent on waypoint positions:
       For each interior waypoint q_k, compute the gradient of path
       length w.r.t. q_k:
           ∂L/∂q_k = (q_k - q_{k-1})/‖q_k - q_{k-1}‖
                    + (q_k - q_{k+1})/‖q_k - q_{k+1}‖
       Then nudge q_k along -∂L/∂q_k (toward the midpoint of its
       neighbours), rejecting moves that cause collision.  This is
       projected gradient descent with a feasibility constraint.
"""
...
from config import (
    ...
    SMOOTHING_ITERS, Q_START, Q_GOAL, K_ATT, K_REP,
)
...
class EnhancedAPFRRTPlanner(APFRRTPlanner):
    """
    Enhanced planner: adaptive APF tuning during growth +
    optimization-based smoothing after path extraction.
    """
    ...
    def smooth_path(
        ...
        """
        Two-stage optimizer that minimises total path length
        subject to collision-free constraints.
        """
        ...
        # Stage 2: Projected gradient descent on waypoint positions
        lr = 0.3  # learning rate (blending factor toward midpoint)
        for _ in range(max_iters // 2):
            improved = False
            for k in range(1, len(path) - 1):
                # Gradient of path length w.r.t. q_k points away from midpoint
                mid    = 0.5 * (path[k-1] + path[k+1])
                nudged = (1.0 - lr) * path[k] + lr * mid
    ...
    def plan(
        ...
        for it in range(1, max_iter + 1):
            ...
            if np.linalg.norm(q_new - q_goal) < GOAL_THRESHOLD:
                ...
                if not self.obs.segment_collides(raw_path[-1], q_goal, n_checks=8):
                    raw_path.append(q_goal.copy())
                smooth_path = self.smooth_path(raw_path)
                # Ensure goal is still the final waypoint after smoothing
                if np.linalg.norm(smooth_path[-1] - q_goal) > 0.01:
                    if not self.obs.segment_collides(smooth_path[-1], q_goal, 8):
                        smooth_path.append(q_goal.copy())
                elapsed = time.time() - t0
                return PlanResult(
                    success=True, path=smooth_path, tree_nodes=tree,
